...
// Separate function implementations to avoid callback inference issues
async function fetchPropertiesImpl(options: PropertyQueryOptions = {}): Promise<PropertyData> {
  let query = supabase
    .from('properties')
    .select(`
      *,
...
async function fetchPropertyByIdImpl(id: string): Promise<Property | null> {
...
export const usePropertyData = () => {
  ...
  // Simplified callback definitions without complex type annotations
  const getProperties = useCallback(async (options?: PropertyQueryOptions) => {
    try {
      setLoading(true);
      setError(null);
      return await fetchPropertiesImpl(options);
    } catch (err) {
  ...
  const getPropertyById = useCallback(async (id: string) => {
    try {
      setLoading(true);
      setError(null);
      return await fetchPropertyByIdImpl(id);
    } catch (err) {
